...
//
// LBEAST Rectangular Frame FeatureScript
// 
// Procedurally generates a rectangular frame made of square steel tube.
// Simple frame with no casters, holes, or additional features - just the basic structure.
...
annotation { "Feature Type Name" : "Rectangular Frame" }
export const rectangularFrame = defineFeature(function(context is Context, id is Id, definition is map)
    precondition
    {
        ...
        annotation { "Name" : "Total on X" }
        isInteger(definition.totalOnX, { (unitless) : [1, 1, 20] } as IntegerBoundSpec);
        
        annotation { "Name" : "Total on Y" }
        isInteger(definition.totalOnY, { (unitless) : [1, 1, 20] } as IntegerBoundSpec);
        
        annotation { "Name" : "Total on Z" }
        isInteger(definition.totalOnZ, { (unitless) : [1, 1, 20] } as IntegerBoundSpec);
    }
    {
        ...
        // Duplication parameters: default to 1 if undefined
        if (definition.totalOnX == undefined)
        {
            definition.totalOnX = 1;
        }
        if (definition.totalOnY == undefined)
        {
            definition.totalOnY = 1;
        }
        if (definition.totalOnZ == undefined)
        {
            definition.totalOnZ = 1;
        }
        ...
        // Procedurally duplicate frames based on totalOnX, totalOnY, totalOnZ
        for (var i = 0; i < definition.totalOnX; i += 1)
        {
            for (var j = 0; j < definition.totalOnY; j += 1)
            {
                for (var k = 0; k < definition.totalOnZ; k += 1)
                {
                    // Calculate base offset for this frame instance
                    const offsetX = i * definition.frameWidth;
                    const offsetY = j * definition.frameDepth;
                    const offsetZ = k * definition.frameHeight;
                    const baseOffset = vector(offsetX, offsetY, offsetZ);
                    
                    // Unique ID suffix for this frame instance
                    const instanceId = id + "x" + i + "y" + j + "z" + k;
                    
                    // Create square frame with 4 tubes
                    // Bottom X tube (along X axis at Y=0)
                    const bottomTubeId = instanceId + "bottomX";
                    createTube(context, bottomTubeId, 
                        baseOffset + vector(zero, zero, zero),
                        baseOffset + vector(definition.frameWidth, zero, zero),
                    ...
                    const topTubeId = instanceId + "topX";
                    const topY = definition.frameDepth + definition.tubeWidth;
                    createTube(context, topTubeId,
                        baseOffset + vector(zero, topY, zero),
                        baseOffset + vector(definition.frameWidth, topY, zero),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    
                    // Left Y tube (along Y axis at X=0, offset by halfTube in X and Y)
                    const leftTubeId = instanceId + "leftY";
                    const leftOffsetX = halfTube;
                    const leftOffsetY = halfTube;
                    createTube(context, leftTubeId,
                        baseOffset + vector(leftOffsetX, leftOffsetY, zero),
                        baseOffset + vector(leftOffsetX, leftOffsetY + definition.frameDepth, zero),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    
                    // Right Y tube (along Y axis at X=width, offset by halfTube - tubeWidth = -halfTube in X)
                    const rightTubeId = instanceId + "rightY";
                    const rightOffsetX = definition.frameWidth - halfTube;
                    const rightOffsetY = halfTube;
                    createTube(context, rightTubeId,
                        baseOffset + vector(rightOffsetX, rightOffsetY, zero),
                        baseOffset + vector(rightOffsetX, rightOffsetY + definition.frameDepth, zero),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    ...
                    createTube(context, instanceId + "topBottomX",
                        baseOffset + vector(zero, zero, topZ),
                        baseOffset + vector(definition.frameWidth, zero, topZ),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    
                    // Top X tube (top frame)
                    createTube(context, instanceId + "topTopX",
                        baseOffset + vector(zero, topY, topZ),
                        baseOffset + vector(definition.frameWidth, topY, topZ),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    
                    // Left Y tube (top frame)
                    createTube(context, instanceId + "topLeftY",
                        baseOffset + vector(leftOffsetX, leftOffsetY, topZ),
                        baseOffset + vector(leftOffsetX, leftOffsetY + definition.frameDepth, topZ),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    
                    // Right Y tube (top frame)
                    createTube(context, instanceId + "topRightY",
                        baseOffset + vector(rightOffsetX, rightOffsetY, topZ),
                        baseOffset + vector(rightOffsetX, rightOffsetY + definition.frameDepth, topZ),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    
                    // Create vertical corner posts
                    // Posts connect bottom frame to top frame
                    ...
                    createTube(context, instanceId + "post1",
                    ...
                    createTube(context, instanceId + "post2",
                    ...
                    createTube(context, instanceId + "post3",
                        baseOffset + vector(postOffsetX, post3OffsetY, postStartZ),
                        baseOffset + vector(postOffsetX, post3OffsetY, postEndZ),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                    
                    // Post 4: Corner offset on both X and Y (back-right)
                    createTube(context, instanceId + "post4",
                        baseOffset + vector(post2OffsetX, post3OffsetY, postStartZ),
                        baseOffset + vector(post2OffsetX, post3OffsetY, postEndZ),
                        halfTube, halfInner, definition.tubeWidth, definition.tubeWallThickness);
                }
            }
        }
